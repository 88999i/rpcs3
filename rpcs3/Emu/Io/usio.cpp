...
void usb_device_usio::translate_input_taiko()
{
	...
	const auto translate_from_pad = [&](usz pad_number, usz player)
	{
		...
		if (const auto& pad = ::at32(handler->GetPads(), pad_number); pad->is_connected() && !pad->is_copilot() && is_input_allowed())
		{
			...
			cfg->handle_input(pad, false, [&](const auto& value, bool& /*abort*/)
			{
				...
				case usio_btn::taiko_hit_side_right:
					...
					// keyboard_pad_handler::process() and consumed from g_taiko_queue
		...
		else if (player < 2)
		{
			...
			// queued hits so a reconnect doesn't fire a stale hit or start on the wrong
			// alternating value.
		...
		if (player < 2)
		{
			...
			// Queue consumption: exactly one queued hit per lane per call, same as the
			// known-good usio1.cpp. Hits are pushed into g_taiko_queue at raw input-poll
			// time by keyboard_pad_handler::process(), independently of how often this
			// function gets called, so draining one entry here is enough to report every
			// hit with minimal added latency and without double-firing.
			std::lock_guard<std::mutex> queue_lock(g_taiko_mutex);
			for (usz lane = 0; lane < 4; lane++)
			{
